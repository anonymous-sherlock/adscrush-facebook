    ...
    <>
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <Button variant="outline" role="combobox" aria-expanded={open} aria-label="Select a account" className={cn("w-[200px] justify-between", className)}>
            <Avatar className="mr-2 h-5 w-5">
              <AvatarImage src={`https://avatar.vercel.sh/${group.accounts.id}.png`} alt={selectedTeam.label} className="" />
            ...
            <CommandList>
              ...
              <CommandGroup key={group.label} heading={group.label}>
                <Link href={`/accounts/${group.accounts.id}`} key={group.accounts.id}>
                  ...
                  >
                    <Avatar className="mr-2 h-5 w-5">
                      <AvatarImage src={`https://avatar.vercel.sh/${group.accounts.id}.png`} alt={group.accounts.label} className="" />
                      <AvatarFallback>{selectedTeam.label.slice(0, 1)}</AvatarFallback>
                    </Avatar>
                    {group.accounts.label}
                    <CheckIcon className={cn("ml-auto h-4 w-4", selectedTeam.id === group.accounts.id ? "opacity-100" : "opacity-0")} />
                  </CommandItem>
                </Link>
              </CommandGroup>
            </CommandList>
