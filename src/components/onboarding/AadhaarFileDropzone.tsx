...
export const AadhaarFileDropzone = () => {
    const { toast } = useToast()
    const { upload, isError } = useFileUpload();
    const { images, setImages } = useAadhaarImages()


    const [isUploading, setIsUploading] = useState<boolean>(false)
    const [uploadProgress, setUploadProgress] = useState<number>(0)

    const { getValues, setError, register, setValue, } = useFormContext<z.infer<typeof FBOnboardingSchema>>();


    useEffect(() => {
        const validImages = FBOnboardingSchema.shape.primaryID.shape.files.safeParse(images)
        if (!validImages.success) {
            setError('primaryID.files', {
                type: 'manual',
                message: `${validImages.error.formErrors.formErrors[0]} files are required`,
            });
        } else {
            setValue('primaryID.files', images, {
                shouldValidate: true,
            });
        }
        ...
        onDrop: async (acceptedFile, fileRejections) => {
            if (images.length === 0 || images.length < 3) {
                ...
                if (acceptedFile.length === 0) {
                    return toast({
                        title: "Upload failed",
                        description: "Only jpg, png, svg, and pdf files are allowed.",
                        variant: "destructive",
                    });
                }
